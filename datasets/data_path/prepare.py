...
def solve_inout(root):
    dataset_path = 'InOutDoor/images/train'
    data_root = os.path.join(root, dataset_path)
    ...
    frames = []
    for video_name in video_paths:
        #if video_name != "seq0" and video_name != "seq3":
        if video_name == "seq3":
            #print(video_name)
            files = os.listdir(os.path.join(data_root, video_name, 'img1'))
            files.sort()
            #for i in range(1, len(files) + 1):
            #    frames.append(os.path.join(dataset_path, video_name, 'img1', '%06d.png' % i))
            for img in files:
                if img != ".DS_Store":
                    frames.append(os.path.join(dataset_path, video_name, 'img1', img))
    return frames
...
root = '../' 
save_path = './inout_seq3_new.train' 
dataset_list = ['InOutDoor', ]
