...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'InOutDoor/images/train', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'InOutDoor/images/train', self.seq_num, 'img1', _) for _ in img_list if
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'InOutDoor/images/train')
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            # InOutDoor has some corrupted images 
            if cv2.imread(self.img_list[i]) is None:
                print("skip:",self.img_list[i])
                continue
